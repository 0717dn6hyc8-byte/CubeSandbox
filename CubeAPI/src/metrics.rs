...
mod tests {
    ...
    #[test]
    fn business_metrics_render_success_and_failure() {
    ...
    #[tokio::test]
    async fn record_business_call_captures_result_and_error_code() {
        let sink = Arc::new(RecordingBusinessMetricsSink::default());
        assert!(
            record_business_call(sink.clone(), BusinessOperation::SandboxCreate, async {
                Ok::<(), AppError>(())
            })
            .await
            .is_ok()
        );
        let error = record_business_call(sink, BusinessOperation::SandboxDestroy, async {
