...
local function labels(route, method, result, extra)
    local base = string.format('method="%s",result="%s",route="%s"',
        escape_label_value(method), escape_label_value(result), escape_label_value(route))
    if extra then
        return extra .. "," .. base
    end
    return base
...
function _M.finish_current_request(status_override)
    ...
    _M.finish(route, method, result, ngx.var.request_time)
    if ngx.ctx then
...
function _M.render_prometheus()
    local d = dict()
    local out = {
        "# HELP cube_proxy_request_total CubeProxy request total by route, method and result",
        "# TYPE cube_proxy_request_total counter",
        "# HELP cube_proxy_inflight_requests CubeProxy in-flight requests by route",
        "# TYPE cube_proxy_inflight_requests gauge",
    }
    if not d then
    ...
    for _, key in ipairs(keys) do
        ...
        if kind == "total" then
            table.insert(out, string.format("cube_proxy_request_total{%s} %s", labels(route, method, result), tostring(value)))
        elseif kind == "inflight" then
            table.insert(out, string.format('cube_proxy_inflight_requests{route="%s"} %s', escape_label_value(route), tostring(value)))
        end
    end
    ...
    for _, key in ipairs(keys) do
        ...
        local value = d:get(key) or 0
        if kind == "bucket" then
            table.insert(out, string.format('cube_proxy_request_duration_seconds_bucket{%s} %s', labels(route, method, result, 'le="' .. escape_label_value(suffix) .. '"'), tostring(value)))
        end
    end
    for _, key in ipairs(keys) do
        local kind, route, method, result = parse_key(key)
        local value = d:get(key) or 0
        if kind == "sum" then
            table.insert(out, string.format("cube_proxy_request_duration_seconds_sum{%s} %s", labels(route, method, result), tostring(value)))
        elseif kind == "count" then
            table.insert(out, string.format("cube_proxy_request_duration_seconds_count{%s} %s", labels(route, method, result), tostring(value)))
        end
    end
    return table.concat(out, "\n") .. "\n"
end
